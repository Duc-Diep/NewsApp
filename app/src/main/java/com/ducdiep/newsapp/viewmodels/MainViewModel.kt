...
class MainViewModel: ViewModel() {
    ...
    fun fetchListNews(country:String,apiKey:String){
        ...
        NewsRepository.getInstance().getTopNews(country,apiKey){ isSuccess: Boolean, response: ResponseNews? ->
        ...
        if (listArticle.value!=null){
            ...
            if (key==""){
                ...
                for (element in listArticle.value!!){
                    if (element.title.toLowerCase().contains(key)||element.description.toLowerCase().contains(key)||element.source.name.toLowerCase().contains(key)){
